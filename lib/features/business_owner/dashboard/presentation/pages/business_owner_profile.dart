...
class _BusinessOwnerProfilePageState extends State<BusinessOwnerProfilePage> {
  ...
  final DashboardRemoteDataSource _dataSource = sl<DashboardRemoteDataSource>();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _infoRow(
            'Phone',
            isLoading ? 'Loading...' : (profile?.phone ?? 'N/A'),
            textColor,
            subtleText,
          ),
          const SizedBox(height: 12),
          _infoRow(
            'Location',
            isLoading ? 'Loading...' : (profile?.location ?? 'N/A'),
            textColor,
            subtleText,
          ),
          if (profile?.companyName != null) ...[
            const SizedBox(height: 12),
            _infoRow('Business', profile!.companyName!, textColor, subtleText),
          ],
        ],
      ),
    );
