...
class BusinessOwnerMessageDetailsPage extends StatefulWidget {
  final Conversation conversation;
  final String userEmail;

  const BusinessOwnerMessageDetailsPage({
    super.key,
  ...
  @override
  State<BusinessOwnerMessageDetailsPage> createState() =>
      _BusinessOwnerMessageDetailsPageState();
}

class _BusinessOwnerMessageDetailsPageState
    extends State<BusinessOwnerMessageDetailsPage>
  ...
  Future<void> _initializeChat() async {
    ...
    if (widget.conversation.rocketChatRoomId == null ||
        widget.conversation.rocketChatRoomId!.isEmpty) {
      context.read<ChatBloc>().add(
        InitializeChatRoomEvent(widget.conversation.id),
      );
      // The BlocListener will handle connecting to WebSocket after room is initialized
    } else {
      // Room already exists, connect directly
      context.read<ChatBloc>().add(
        ConnectWebSocket(
    ...
    return BlocListener<ChatBloc, ChatState>(
      listener: (context, state) {
        if (state is RoomInitialized) {
          // Room initialized, now connect to WebSocket
          context.read<ChatBloc>().add(
            ConnectWebSocket(
          ...
          _scrollToBottom();
        } else if (state is RoomError || state is ChatConnectionError) {
